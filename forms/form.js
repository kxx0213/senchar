				...
				items : [{
					...
					items : [{
						...
						handler : function(){
							...
							var loadValues = formPanel.getValues();  //formPanel.getValues() = 폼의 모든 값을 가져와서 name으로 해당 컨포넌트의 값을 불러올 수 있게 된다.
							...
							// id를 이용하여 formPanel에 입력한 값을 저장하는 방법 
                      	    var name = Ext.getCmp('nameId').getValue();   // ID로 값을 바로 불러오는 방법! 최고 좋다.!
                      	    var birthday = Ext.getCmp('birthday').getValue(); 
                      	    var mili = Ext.getCmp('mili').getChecked(); 
                      	    var career = Ext.getCmp('career').getValue(); 
                      	    var gender = Ext.getCmp('gender').getRecord( ).get('text') ; //젠더는 값을 얻어오는 방식이 좀 다르다.
...
//                      	    		url:'formdata.jsp',
//                      	    		method : 'POST',
//                      	    		//params: formPanel.getValues(),  //한꺼번에 싹~~넘어간다. 와우!
//                      	    		
//                      	    		params : [
